...
// constructor
JetCleanser::JetCleanser(fastjet::JetDefinition subjet_def, cleansing_mode cmode, input_mode imode) {
...
JetCleanser::JetCleanser(double rsub, cleansing_mode cmode, input_mode imode) {
	fastjet::JetDefinition subjet_def(fastjet::kt_algorithm, rsub);
	_subjet_def = subjet_def;
	_rsub = rsub;
	_cleansing_mode = cmode;
	_input_mode = imode;
...
void JetCleanser::SetGroomingParameters(double fcut, int nsjmin) {
	if ( fcut < 0 || fcut > 1 ) throw fastjet::Error("SetGroomingParameters(): fcut must be >= 0 and <= 1");
	_fcut = fcut;
	_nsjmin = nsjmin;
}
...
void JetCleanser::SetLinearParameters(double g0_mean) {
	if ( g0_mean < 0 || g0_mean > 1 ) throw fastjet::Error("SetLinearParameters(): g0_mean must be >= 0 and <= 1");
...
void JetCleanser::SetGaussianParameters(double g0_mean, double g1_mean, double g0_width, double g1_width) {
	if ( g0_mean < 0 || g0_mean > 1 ) throw fastjet::Error("SetGaussianParameters(): g0_mean must be >= 0 and <= 1");
	if ( g1_mean < 0 || g1_mean > 1 ) throw fastjet::Error("SetGaussianParameters(): g1_mean must be >= 0 and <= 1");
	if ( g0_width < 0 || g0_width > 1 ) throw fastjet::Error("SetGaussianParameters(): g0_width must be >= 0 and <= 1");
	if ( g1_width < 0 || g1_width > 1 ) throw fastjet::Error("SetGaussianParameters(): g1_width must be >= 0 and <= 1");
...
/////////////////////////////
// cleanse
fastjet::PseudoJet JetCleanser::operator()(const fastjet::PseudoJet & jet, 
			const std::vector<fastjet::PseudoJet> & tracks_lv, 
			const std::vector<fastjet::PseudoJet> & tracks_pu) const {

	if ( _input_mode != input_nc_together ) throw fastjet::Error("result(): This operator is only defined for input_nc_together mode");

	// get constituents
	if ( !(jet.has_constituents()) ) return fastjet::PseudoJet();
	std::vector<fastjet::PseudoJet> constituents_all = jet.constituents();

	// prepare for clustering
	std::vector< std::vector<fastjet::PseudoJet> > follow_sets;
	follow_sets.push_back( constituents_all );
	follow_sets.push_back( tracks_lv );
	follow_sets.push_back( tracks_pu );

	// get subjets
	std::vector< std::vector<fastjet::PseudoJet> > sets = ClusterSets(_subjet_def, constituents_all, follow_sets);
	std::vector<fastjet::PseudoJet> subjets_all, subjets_tracks_lv, subjets_tracks_pu;
	subjets_all       = sets[0];
	subjets_tracks_lv = sets[1];
	subjets_tracks_pu = sets[2];

	// rescale subjets
	std::vector<fastjet::PseudoJet> rescaled_subjets_all;
	for (unsigned i=0; i<subjets_all.size(); i++){
		...
		fastjet::PseudoJet rescaled_subjet_all = RescalePseudoJetConstituents( subjets_all[i], s_factor );
	...
	std::vector<fastjet::PseudoJet> trimmed_subjets_all;
...
/////////////////////////////
// cleanse
fastjet::PseudoJet JetCleanser::operator()(const std::vector<fastjet::PseudoJet> & neutrals_all,
			const std::vector<fastjet::PseudoJet> & tracks_lv, 
			const std::vector<fastjet::PseudoJet> & tracks_pu) const {

	if ( _input_mode != input_nc_separate ) throw fastjet::Error("result(): This operator is only defined for input_nc_separate mode");
	...
	fastjet::PseudoJet jet = join(particles_all);
	...
	// get subjets
	std::vector< std::vector<fastjet::PseudoJet> > sets = ClusterSets(_subjet_def, particles_all, follow_sets);
	std::vector<fastjet::PseudoJet> subjets_all, subjets_neutrals_all, subjets_tracks_lv, subjets_tracks_pu;
	subjets_all          = sets[0];
	...
	for (unsigned i=0; i<subjets_all.size(); i++){
		...
		fastjet::PseudoJet rescaled_subjet_ntrl = RescalePseudoJetConstituents( subjets_neutrals_all[i], s_factor );
		fastjet::PseudoJet rescaled_subjet_all = join( rescaled_subjet_ntrl, subjets_tracks_lv[i] );
		if ( rescaled_subjet_all != 0.0 ) rescaled_subjets_all.push_back( rescaled_subjet_all );
	}
...
void JetCleanser::_CheckRescalingValues(double & pt_all, const double & ptc_lv, const double & ptc_pu) const {
	...
	if ( ratio > 1.05 ) throw fastjet::Error("_CheckRescalingValues: ptc_lv + ptc_pu is more than 5\% larger than pt_all");
...
double JetCleanser::_GetSubjetRescaling_nctogether(double pt_all, double ptc_lv, double ptc_pu) const {
	...
	if ( _cleansing_mode == jvf_cleansing ) {
		scale = ptc_lv > JetCleanser::jc_zero ? ptc_lv / ( ptc_lv + ptc_pu ) : 0.0;
	}

	// linear mode
	else if ( _cleansing_mode == linear_cleansing ) {
		if ( _linear_gamma0_mean < 0 ) throw fastjet::Error("Linear cleansing parameters have not been set yet.");
	...
	// gaussian mode
		...
		if ( _gaussian_gamma0_mean < 0 || _gaussian_gamma1_mean < 0 || _gaussian_gamma0_width < 0 || _gaussian_gamma1_width < 0 )
		  throw fastjet::Error("Gaussian cleansing parameters have not been set yet.");
		_CheckRescalingValues(pt_all,ptc_lv,ptc_pu);
	...
	else throw fastjet::Error("_GetSubjetRescaling: Current cleansing mode is not recognized.");
...
std::vector< std::vector<fastjet::PseudoJet> > ClusterSets(const fastjet::JetDefinition & jet_def, 
			...
			const double &ptmin) {
	// start set
	std::vector<fastjet::PseudoJet> full_set;
	...
	for (unsigned i=0; i<follow_sets.size(); i++){
		std::vector<fastjet::PseudoJet> current_set = follow_sets[i];
		for (unsigned j=0; j<current_set.size(); j++){
			FollowSetGhostInfo* ghost_info = new FollowSetGhostInfo(i,j);
			fastjet::PseudoJet ghost_pseudojet = 1.0e-60 * current_set[j];
	...
	for (unsigned i=0; i<follow_sets.size(); i++){
		std::vector<fastjet::PseudoJet> current_set;
		for (unsigned j=0; j<jets.size(); j++){ current_set.push_back( join( fastjet::PseudoJet() ) ); }
	...
	for (unsigned i=0; i<jets.size(); i++){
		...
		for (unsigned j=0; j<constituents.size(); j++){
			if ( constituents[j].has_user_info<FollowSetGhostInfo>() ){
				...
				std::vector<fastjet::PseudoJet> current_set = follow_sets[set_id];
...
std::vector<fastjet::PseudoJet> RescalePseudoJetVector(const std::vector<fastjet::PseudoJet> & jets, const double s_factor) {
	std::vector<fastjet::PseudoJet> rescaled_jets;
...
fastjet::PseudoJet RescalePseudoJetConstituents(const fastjet::PseudoJet & jet, const double s_factor) {
	...
	std::vector<fastjet::PseudoJet> rconstituents = RescalePseudoJetVector( constituents, s_factor );
...
void JetCleanser::_RunTestRescaling(double pt_all, double ptc_lv, double ptc_pu) const {
	...
	catch(fastjet::Error e) { s_factor = -1.0; }
